...
public class StudentAPI_Required_Optional_Test extends JerseyTest {
	...
	protected Application configure() {
		...
		return new ResourceConfig(CourseAPI_Required_Optional.class, StudentAPI_Required_Optional.class);
	}
	...
	public void create_Update_Get_Delete_Student_Sucess() {
		...
		try {
			...
			// get student
			SAReader_Required_Optional student = target(STUDENT_3_URL).queryParam("external_course_id", "1")
					.queryParam("external_student_id", "1").request().get(SAReader_Required_Optional.class);
			assertEquals("verify  inserted record", "Senior", student.getS_year());

			// update student
			sa = new SAReader_Required_Optional();
			sa.setExternal_course_id("1");
			sa.setExternal_student_id("1");
			sa.setS_placement_score(3.0);
			sa.setS_year("Junior");
			...
			student = target(STUDENT_3_URL).queryParam("external_course_id", "1").queryParam("external_student_id", "1")
					.request().get(SAReader_Required_Optional.class);
