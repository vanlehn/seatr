package com.asu.seatr.handlers.analyzer.unansweredtasks;
...
public class RecommTaskHandler_UnansweredTasks {
	...
	private static void fillRecommTask(Student stu,Course course, int numToFilled){
		...
		try{
			...
			for(int i=0;i<numToFilled;i++){
				...
				RecommTask_UnansweredTasks recom=new RecommTask_UnansweredTasks();
	...
	public static void initRecommTasks(int num){
		...
		Session session=sf.openSession();
		session.beginTransaction();
		Query q=session.createQuery("delete from RecommTask_UnansweredTasks");
		q.executeUpdate();
		session.getTransaction().commit();
	...
	public static void completeATask(Student stu, Course course, Task task){
		...
		Session session=sf.openSession();
		Criteria cr=session.createCriteria(RecommTask_UnansweredTasks.class);
		cr.add(Restrictions.eq("student", stu));
		cr.add(Restrictions.eq("task", task));
		cr.add(Restrictions.eq("course", course));
		List<RecommTask_UnansweredTasks> recommList=cr.list();
		if(recommList.size()>0){
			...
			try{
				cr=session.createCriteria(RecommTask_UnansweredTasks.class);
				cr.add(Restrictions.eq("student", stu));
				cr.add(Restrictions.eq("course", course));
				recommList=cr.list();
				session.close();
